...
void down(sem_t *sem) {
    sem_wait(sem); // decrementa di 1 il semaforo , se questo è 0 il thread chiamante viene bloccato;
}

void up(sem_t *sem) {
    sem_post(sem); // incrementa di 1 il semaforo;
}
...
int main() {
    ...
    sem_init(&mutex, 0, 1); // pointer semaforo; pshared: 0 per i thread , 1 per i processi; valore contatore;
    sem_init(&empty, 0, N); //buffer vuoto
    sem_init(&full, 0, 0); //buffer vuoto

    // Creazione dei thread
    pthread_create(&prod, NULL, producer, NULL);
    pthread_create(&cons, NULL, consumer, NULL);
    //pthri, const pthread_attr_t *restrict attr, void *(*start_routine)(void *), void *restrict arg)
    //pthrei, void **thread_return)
